...
write_to_log -ServerInput $ServerInput -log_msg "    - Network interface card (NIC) adapter type is VMXNET3."
...
Try {
    ...
    write_to_log -ServerInput $ServerInput -log_msg "    Success - Network interface card (NIC) adapter set successfully."
} 
catch [System.Exception] {
    write_to_log -ServerInput $ServerInput -log_msg $_.Exception.Message -is_error $true
}
# Step 06 - Set VM Tools Current
write_to_log -ServerInput $ServerInput -log_msg "Step 06 - Set VM Tools Current"
Try {
    ...
    if ($ToolsInstalled -eq "guestToolsCurrent") {
        write_to_log -ServerInput $ServerInput -log_msg "    Success - Step  06 - Set VM Tools Current."
    }
} 
catch [System.Exception] {
    write_to_log -ServerInput $ServerInput -log_msg $_.Exception.Message -is_error $true
